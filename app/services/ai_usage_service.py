...
def get_user_ai_usage(user_id, filter_data=None):
    ...
    query = AIUsageModel.query.filter(AIUsageModel.user_id == user_id)
...
def get_usage_stats(user_id=None):
    ...
    if user_id:
        ...
        usage_count = AIUsageModel.query.filter(AIUsageModel.user_id == user_id).count()
        user = UserModel.query.filter_by(id=user_id).first()
        ...
        # Global stats
        total_cost = db.session.query(db.func.sum(AIUsageModel.cost)).scalar() or 0.0
        usage_count = AIUsageModel.query.count()
        total_users = AIUsageModel.query.distinct(AIUsageModel.user_id).count()
